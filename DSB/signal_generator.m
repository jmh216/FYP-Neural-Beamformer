...
n = 16000;               % Number of samples for RIR
...
%     for figure_plot = 1
%     figure;
%     plot3(r(:,1),r(:,2),r(:,3),'o'); % scatter3(s(:,1),s(:,2),s(:,3));
%     hold on;
%     plot3(s(:,1),s(:,2),s(:,3),'rx'); % scatter3(r(:,1),r(:,2),r(:,3)); 
%     legend('Microphones','Target Signal Source');
...
%plot rir of each channel seperately and together
%--------------------------------------------------------------------------
%     for figure_plot = 2
%     figure;
%     for i = 1:Mic_number
...
%     xlabel("Samples"); ylabel("Amplitude");
%     legend('Mic 1 RIR','Mic 2 RIR','Mic 3 RIR','Mic 4 RIR','Mic 5 RIR','Mic 6 RIR');
%     end
%--------------------------------------------------------------------------

%plot rir variation with change of reverberation time beta
%--------------------------------------------------------------------------
%     for figure_plot = 3
%         figure;
%     for i = 1:4
...
%     audio_out = ConvWithRIR(audio_in',RIR);
%     figure;
...
%     subplot(413); plot(t,audio_out(2,:)); title('out(n)');xlabel('Time [Seconds]');ylabel('Amplitude');
%     subplot(414); plot(t,audio_out'); title('out(n)');xlabel('Time [Seconds]');ylabel('Amplitude');
%     grid on;
% 
%     SNR=5; % dB
%     audio_out(1,:)=awgn(audio_out(1,:),SNR,'measured');
%     audio_out(2,:)=awgn(audio_out(2,:),SNR,'measured');
%     audio_out(3,:)=awgn(audio_out(3,:),SNR,'measured');
%     audio_out(4,:)=awgn(audio_out(4,:),SNR,'measured');
%     audio_out(5,:)=awgn(audio_out(5,:),SNR,'measured');
%     audio_out(6,:)=awgn(audio_out(6,:),SNR,'measured');
% 

    ...
    methods
        ...
        function[] = prepareData(obj,req_fs,varargin)
            ...
            for iSensor = 1:obj.nSensors
                ...
                % fix for mode strength so low that it becomes nan
                b(1,isnan(b(1,:))) = 0;
        ...
        function[H,rel_src_az,rel_src_inc] = getFrequencyResponseForSrc(obj,src_az,src_inc,varargin)
            ...
            % dimension
            %obj.sensorInverseSftMatrix: [nFreq nSH 1 nChan]
       ...
       %            [1,4,3,2]); %profile: 52.06
            ...
            % protect against big matrices
            H = zeros(nFreq,1,nDOA,obj.nSensors);
            for iDOA = 1:nDOA
                H(:,:,iDOA,:) = sum(bsxfun(@times,Anm(:,:,iDOA), obj.sensorInverseSftMatrix),2);
            end
            H = permute(H,[1,4,3,2]);

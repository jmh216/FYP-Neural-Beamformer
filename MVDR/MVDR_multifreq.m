...
source_resolution = 5;  % Angle difference of each source
...
for frequency_index = 1:length(source_frequency)
    ...
    % sinusoidal input
    source = cos(2*pi*source_frequency(frequency_index)*t); 
...
final_out_gain(frequency_index,:) = output_gain;
end
...
% title({["MVDR beampattern for 1000 Hz signal" ],["Look direction " + num2str(steer_angle) + " degree - Directional noise 45 and 150 degree" ]});
...
% title({["Polar plot of MVDR beampattern for 1000 Hz signal" ],["Look direction " + num2str(steer_angle) + " degree - Directional noise 45 and 150 degree"]});
...
switch params.noise_model
    ...
    case 'directional'           
        noise_angle = 180; % 90 degree-[0 1 0] ; 0 degree-[1 0 0]
...
%         h = ema.getImpulseResponseForSrc(deg2rad(70),deg2rad(0));
    ...
    case 'multi-directional'           
            noise_angle_1 = 45; % 90 degree-[0 1 0] ; 0 degree-[1 0 0]
            unit_noise_position = [cosd(noise_angle_1) sind(noise_angle_1) 0]; 
            noise_cart = unit_noise_position;  
            [noise_az,noise_inc,~] = mycart2sph(noise_cart);
            
            noise_angle_2 = 150; % 90 degree-[0 1 0] ; 0 degree-[1 0 0]
            unit_noise_position = [cosd(noise_angle_2) sind(noise_angle_2) 0]; 
            noise_cart = unit_noise_position;  
            [noise_az2,noise_inc2,~] = mycart2sph(noise_cart);

            h1 = ema.getImpulseResponseForSrc(noise_az,noise_inc);
            h2 = ema.getImpulseResponseForSrc(noise_az2,noise_inc2);
    %         h = ema.getImpulseResponseForSrc(deg2rad(70),deg2rad(0));
            h = h1 + h2;

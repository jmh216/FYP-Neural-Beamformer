    ...
    methods
        ...
        function[] = setPoseRollPitchYaw(obj,roll,pitch,yaw)
            ...
            if any(isnan([roll,pitch,yaw]))
                error('roll,pitch and yaw must be defined')
            end
        ...
        function[src_az,src_inc] = planeWaveDoaWrtWorld(obj,rel_src_az,rel_src_inc)
            % planeWaveDoaWrtWorld converts directions of arrival input with
            % respect to the current array orientation into world co-ordinates
            % plane wave means that the DOA is independent of source
            % distance and of any array translation
            % based on code from dir_wrt_rotated_axes.m
            % requires voicebox
            [~,must_be_one_az] = size(rel_src_az);
            [~,must_be_one_inc] = size(rel_src_inc);
            
            if ~isequal(must_be_one_az,1) || ~isequal(must_be_one_inc,1)
            ...
            if obj.poseAxes==eye(3)
                ...
                % DOAs as unit vectors pointing towards DOA
                doa_vec = mysph2cart(rel_src_az,rel_src_inc,ones(size(rel_src_az)));
                doa_vec = rotqrvec(obj.poseQuaternion,doa_vec.').';
                [src_az,src_inc,~] = mycart2sph(doa_vec);
            end
        end    

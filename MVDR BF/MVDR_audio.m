...
for i = 1:2
    ...
    for idoa = 1:length(noise_az_deg)
        ...
        diffuse_noise = diffuse_noise + 1/5000 * fftfilt(squeeze(h_circ(:,:,idoa)),new_mono_noise);
    end
    ...
    bf_out(:,i) = sum(fftfilt(filtCoefs,mixed),2);
end
...
for i = 1:2
    ...
    [nosie_az,noise_inc,~] = mycart2sph(unit_noise_position);
    h_noise = ema.getImpulseResponseForSrc(nosie_az,noise_inc);
    
    mic_speech = fftfilt(h,speech);
    mic_noise = fftfilt(h_noise,audio_inter);

    mixed = mic_speech + mic_noise;
